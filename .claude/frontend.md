...
This file describes the **website repo**: `C:\Users\HP\bari`. All implementation in this document happens there.
...
- All frontend work (routes, components, UI, build, lint) → **`C:\Users\HP\bari`**.
- **Never assume `C:\Bari` is the website repo** — no Next.js source lives there.
- **Never modify website source files under `C:\Bari`.** The frontend JSON the site renders lives in `C:\Users\HP\bari\src\data\comparisons\` (generated from BSIP2 outputs in `C:\Bari`, then copied over).
- Before implementing, **confirm the working directory is `C:\Users\HP\bari`**.
