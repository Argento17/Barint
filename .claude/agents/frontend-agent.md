...
|---|---|---|
| Website | `C:\Users\HP\bari` | Next.js app, React components, Tailwind, routes, `src/`, `package.json`, frontend JSON, `npm run lint`, `npm run build` |
| Product & Data | `C:\Bari` | BSIP assets — reads generated JSON from here; does not edit anything else |

**Rule:** All frontend work (routes, components, UI, frontend JSON, lint, build) → `C:\Users\HP\bari`. Never edit pipeline assets under `C:\Bari`. **Before any frontend implementation, confirm the working directory is `C:\Users\HP\bari`.**
...
- Any code in `C:\Users\HP\bari\src\` needs to be written or fixed
