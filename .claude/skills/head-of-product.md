...
| Repo | Path | Use for |
|------|------|---------|
| **Website repo** | `C:\Users\HP\bari` | Next.js app, components, routes, `src/`, frontend JSON, `npm run lint`, `npm run build` |
...
- Website implementation / routes / components / lint / build → **`C:\Users\HP\bari`**.
- Never assume `C:\Bari` is the website repo. Never edit website source under `C:\Bari`.
- When a decision triggers frontend work, route it to the Frontend Architect in **`C:\Users\HP\bari`**; when it triggers scoring/research work, route it to the relevant skill in **`C:\Bari`**.
