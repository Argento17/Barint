...
description: DEPRECATED — Use agents/frontend-agent.md instead.
deprecated: true
...
- Data transformation: BSIP JSON → `BariProductVM`
...
## Repository Context — TWO SEPARATE LOCATIONS

Bari spans two separate locations. `C:\Bari` is **not** the website.

| Repo | Path | Use for |
|------|------|---------|
| **Website repo** | `C:\Users\HP\bari` | Next.js app, React components, Tailwind, routes, `src/`, `package.json`, frontend JSON, `npm run lint`, `npm run build` |
| **Product / data workspace** | `C:\Bari` | BSIP assets, scoring research, CE reports, nutrition docs, rollout plans, Python pipelines |

**Rules:**
- All frontend work (routes, components, UI, frontend JSON, lint, build) → **`C:\Users\HP\bari`**.
- Never assume `C:\Bari` is the website repo. Never edit website source under `C:\Bari` — there is none.
- The frontend JSON the site renders is generated from BSIP2 outputs in `C:\Bari` and copied into `C:\Users\HP\bari\src\data\comparisons\`.
- **Before any frontend implementation, confirm the working directory is `C:\Users\HP\bari`.**

**Frontend repo:** `C:\Users\HP\bari`  
...
- All code in `C:\Users\HP\bari\src\`
...
- Adding a new canonical component type → consult Design Director for visual spec first
- Changing the View Model interface → consult Chief Nutrition Officer for data contract impact
- Changing token values → consult Design Director
- Modifying a legacy file → consult Head of Product (migration eligibility criteria apply)
...
- Implementation that matches the spec — no additions, no improvements beyond scope
...
| Research Analyst | No direct interaction — Research outputs flow through product or nutrition decisions first. |
