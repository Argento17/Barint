...
## Content sign-off (HARD RULE, owner ruling 2026-06-20)
- **Nothing consumer-facing reaches the owner's eyes without TWO sign-offs: the Content Agent AND the Adversarial QA / Red-Team gate.** Every owner-facing string — insightLine, rowVerdict, expansion copy, hero/prologue, category notes, card copy — is a *draft* until both have signed off. No exceptions, every category, forever. A page shown to the owner on un-signed-off copy is a process failure regardless of how the copy reads.
- **The orchestrator (Opus/C4) must NOT author or ship content inline.** Orchestrator-written copy is unapproved draft by definition. (This is the tracked drift that caused the 2026-06-20 snacks failure — inline-Opus copy reached the owner with zero sign-off; see [[content_signoff_hard_rule]].)
- **Authoring engines (updated by Capability Router v5, owner-approved 2026-07-10, superseding the 2026-06-20 lane-agnostic list):** consumer Hebrew copy routes to the CONTENT capability = Claude models (primary claude-fable-5). The former Cursor/Grok author lanes are KILLED; Gemini and Codex are prohibited from consumer copy by Router v5 (Gemini recites grams/E-codes in Hebrew editorial; builders build). The binding constraint remains the **two-gate sign-off**, never the engine.
...
- **Loop-first (owner directive 2026-07-04).** The system runs as a loop, not a Q&A: convert every would-be owner question into (a) a reversible default decision logged in the registry, or (b) a background dispatch to the deciding agent (Product / Nutrition / C3) while other ready work continues. Inter-agent approval gates are satisfied by dispatching the approver, never by owner arbitration. Owner-relevant items batch into ONE end-of-run digest — no mid-run drip.
...
- **Two-speed merge policy (owner ruling 2026-07-10).** The orchestrator merges to origin/master **autonomously** when the change is **internal** (ops, CI, registry, router, tooling — nothing a site visitor sees) AND all checks/selftests are green; it runs the relevant selftests in the merge worktree before pushing and logs the merge on the board. **Consumer-facing changes keep the owner merge** — that click doubles as the owner's final look at what ships. Deploy-holding via Vercel Deployment Checks (TASK-571) is the standing prerequisite the owner still needs to click.

## Model routing (Capability Router v5.2 — LAW, owner-approved 2026-07-10, tiers 2026-07-11)
- **Every task routes through the Capability Router:** `01_framework/operations/capability_router_v5.md` (the law) implemented by `03_operations/router/dispatch.py` (`--selftest-table` asserts code==doc). Route by CAPABILITY via the Layer-1 ordered questions (first match wins); models are bound ONLY in Layer 2. PLANNING resolves ambiguity before any implementation. Complexity signals (never file counts) pick heavy vs light builds. Cross-vendor challenge invariant: the challenger is always a different company than the producer.
- **The Bari tier map (owner law 2026-07-11):** SST/strategist = Fable 5 ↔ Sol 5.6 · Heavy builder = Sonnet 5 ↔ Terra 5.6 · Grunt = Haiku 4.5 ↔ Luna 5.6 · Deterministic = scripts. **SST models never do code or grunt work** — strategize/inspect/orchestrate/ideate only. Fallbacks stay within tier, across vendor.
- **Orchestrator default = Opus 4.8, ALWAYS (owner law 2026-07-11).** The main-loop/orchestrator session runs on `claude-opus-4-8`; Opus is also the Adversarial-QA and Claude-side CHALLENGE pin. **Fable 5 and Sol 5.6 engage ONLY via the `/stf` (Strategy Task Force) skill** — the STRATEGY-CONSULT capability — never as the ambient session. In `/stf`, the Opus orchestrator chairs; the Claude seat is a Fable-pinned subagent and the GPT seat is Sol (read-only).
- Builder/grunt pins run on the owner's Codex subscription (terra/luna); evidence research = GPT + web; vision/long-read = Gemini via Antigravity (report-only, pin-gated). **KILLED forever: Grok CLI, cursor-agent, DeepSeek, anonymous free-proxy models.** Every Claude subagent dispatch states its model pin explicitly.
