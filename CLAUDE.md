...
## Model routing (Capability Router v5 — LAW, owner-approved 2026-07-10)
- **Every task routes through the Capability Router v5:** `01_framework/operations/capability_router_v5.md` (the law) implemented by `03_operations/router/dispatch.py` (`--selftest-table` asserts code==doc). Route by CAPABILITY via the Layer-1 ordered questions (first match wins); models are bound ONLY in Layer 2. PLANNING resolves ambiguity before any implementation. Complexity signals (never file counts) pick heavy vs light builds. Cross-vendor challenge invariant: the challenger is always a different company than the producer.
- Pins (2026-07-10): builds/grunt = Codex gpt-5.6 sol/terra/luna on the owner's subscription; evidence research = GPT + web; vision/long-read = Gemini via Antigravity (report-only, currently pin-gated); planning/content/domain judgment = Claude. **KILLED forever: Grok CLI, cursor-agent, DeepSeek, anonymous free-proxy models.** Every Claude subagent dispatch states its model pin explicitly.
