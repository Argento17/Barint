...
TRACES = glob.glob(r"C:\Bari\02_products\cheese_spreads\bsip2_outputs\run_cheese_001\products\**\bsip2_trace.json", recursive=True)
...
OUT = pathlib.Path(r"C:\Bari\02_products\cheese_spreads\factory_run_001\frontend_package.json")
...
package = {
    ...
    "non_authoritative_reason": "QA misroute gate FAILED (47.4% > 5%, QA-CHS-001 router cream-cheese-anchor gap breaks the entire cream-cheese pool) and Nutrition approval pending (A-ceiling withhold). Factory artifact only; NOT shipped to live site. The cottage / white-cheese / labaneh rows route correctly and are reliable; the cream-cheese-pool rows are withheld pending the scoped router fix (run_cheese_002).",
    "rtl": True,
    "language": "he",
