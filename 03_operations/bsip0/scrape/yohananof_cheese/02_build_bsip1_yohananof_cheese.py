"""
BSIP1 Builder — Yohananof Cheese (TASK-210 Phase B).

Source: yohananof_cheese_bsip0_raw_*.json (il_prices + OFF panels, candidate status).
Curation reuses the SAME _curate() logic as run_cheese_003 (byte-identical).
BSIP0 filters (F1-F4, F6) from _shared/bsip0_nutrition.py applied first.

Output: C:\Bari\03_operations\bsip1\run_cheese_yohananof_001\output\bsip1_*.json
"""
...
# Import the live curation + parse code from run_cheese_003 builder (UNCHANGED)
SHUF_BUILDER = pathlib.Path(r"C:\Bari\03_operations\bsip0\scrape\shufersal_cheese")
spec = importlib.util.spec_from_file_location(
    "cheese_builder_003", SHUF_BUILDER / "02_build_bsip1_cheese_003.py")
B = importlib.util.module_from_spec(spec)
spec.loader.exec_module(B)

RAW_DIR = pathlib.Path(r"C:\Bari\02_products\cheese_spreads\bsip0_outputs")
OUT_DIR = pathlib.Path(r"C:\Bari\03_operations\bsip1\run_cheese_yohananof_001\output")
...
def main():
    raws_files = sorted(RAW_DIR.glob("yohananof_cheese_bsip0_raw_*.json"),
                        key=lambda p: p.stat().st_mtime, reverse=True)
    if not raws_files:
        print(f"ERROR: No yohananof_cheese_bsip0_raw_*.json found in {RAW_DIR}")
        return None
    ...
    for raw in deduped:
        ...
        # Reuse live cheese curation ruling (byte-identical to run_cheese_003)
        ...
        subtype, _ = B._classify_subpool(name, ingr_text, nn)
        ...
        included.append({
            "barcode": barcode, "name": name, "subpool": subtype,
