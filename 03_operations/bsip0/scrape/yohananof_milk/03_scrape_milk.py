"""
BSIP0 — Yohananof Milk Scraper
Reads approved_candidates.json, scrapes each product's ingredient/nutrition/allergen tabs.
Output: outputs/yohananof_milk/<barcode>/{ingredients.html, nutrition.html, allergens.html, ...}
Next step: python 04_parse_milk.py
"""
...
def load_approved():
    path = RETAILER_DIR / "approved_candidates.json"
    if not path.exists():
        raise FileNotFoundError(
            f"approved_candidates.json not found: {path}\n"
            "Run: python 02_approve_milk.py"
        )
...
def save_product_image(product, product_dir):
    ...
    if not image_url:
        return {"source_image_url": None, "local_image_file": None, "image_download_error": "missing_image_url"}
    try:
...
def open_product_modal(page, product):
    ...
    if not barcode:
        raise RuntimeError(f"Missing barcode: {name}")
    ...
    raise RuntimeError(f"Product not found after scrolling: {barcode} | {name}")
...
def scrape_one(page, product):
    barcode = clean(product.get("barcode"))
    name = clean(product.get("name"))
    product_dir = RETAILER_DIR / barcode
    ...
    return {"barcode": barcode, "name": name, "status": "scraped", **status}
...
def main():
    ...
    print(f"\nScraping {len(approved)} approved milk products")
    run_report = []
    ...
    with sync_playwright() as p:
        ...
        for product in approved:
            ...
            try:
                ...
                print(f"  OK: ingredients={result['ingredients']} nutrition={result['nutrition']}")
            except Exception as e:
                close_dialog(page)
                run_report.append({"barcode": barcode, "name": name, "status": "failed", "error": str(e)[:300]})
                print(f"  FAILED: {e}")

            save_json(RETAILER_DIR / "run_report.json", run_report)
    ...
    print("Next: python 04_parse_milk.py")
