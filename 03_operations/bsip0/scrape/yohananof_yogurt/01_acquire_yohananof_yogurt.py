"""
BSIP0 Yohananof — Yogurt acquisition (TASK-210 Phase B).

Uses the same il_prices + OFF model as multiretailer_cereals/01_acquire_multiretailer.py:
  - Fetches Yohananof PriceFull catalog via laibcatalog.co.il (no login, price-transparency law)
  - Name-gates for yogurt shelf candidates
  - Pairs each barcode with OFF for nutrition + ingredients (candidate panel, EDPG)

Output: C:\Bari\02_products\yogurt_system\bsip0_outputs\yohananof_yogurt_bsip0_raw_<ts>.json
"""
...
# Yogurt shelf name-gate (broad — BSIP0 filtering does the real quality gate)
YOGURT_INCLUDE = [
    "יוגורט", "יוגרט", "יוגורת", "yogurt",
    "לבן", "גביע יוגורט", "גביע לבן",
    "קפיר", "kefir",
    "דנונה", "danone",  # brand-based
    "עמוק", "אקטיביה", "activia",
    "אקטיב", "ביו", "bio",
    "יופלה", "yoplait",
    "בריטי", "מופלא",
]

# Hard-drop non-yogurt items that share these keywords in mixed-shelf contexts
YOGURT_HARD_DROP = [
    "גלידה", "קרם", "גבינה", "מעדן", "פודינג", "שמנת",
    "קוקוס", "הפשרה", "ממרח",
    "חטיף", "עוגייה", "שוקולד", "ממתק",
    "שמפו", "סבון", "ניקוי",
]

# Cheese shelf name-gate
CHEESE_INCLUDE = [
    "גבינה", "גביש", "קוטג", "קוטג'", "ריקוטה",
    "לאבנה", "ממרח גבינה", "פטה", "בולגרית",
    "מוצרלה", "צהובה", "פרמזן", "שמנת גבינה",
...
CHEESE_HARD_DROP = [
    "גלידה", "שוקולד", "ממתק", "עוגיית",
    "שמפו", "סבון", "ניקוי", "חטיף",
]
...
def extract_weight_g(name: str):
    for pat in (
        ...
        re.compile(r"(\d[\d,.]*)\s*מ[\"']?ל", re.I),
        re.compile(r"(\d[\d,.]*)\s*ml\b", re.I),
    ):
        m = pat.search(name)
        if m:
            try:
...
def acquire(category_name, include_signals, hard_drop, off_cap=200):
    print(f"\n=== Acquiring {category_name} from Yohananof ===")
    ...
    for i, it in enumerate(cand[:off_cap]):
        ...
        if not (p and p.has_panel):
            no_panel += 1
        out.append(off_to_bsip0(it, p, RETAILER_ID, RETAILER_NAME, category_name))
        if i % 20 == 0 and i:
            print(f"  [{i}/{len(cand[:off_cap])}] no-panel so far: {no_panel}")
        time.sleep(0.2)
...
def main():
    ...
    # Yogurt
    yogurt_raw = acquire("yogurt", YOGURT_INCLUDE, YOGURT_HARD_DROP, off_cap=150)
    if yogurt_raw:
        out_path = OUT_DIR / f"yohananof_yogurt_bsip0_raw_{ts}.json"
        out_path.write_text(json.dumps(yogurt_raw, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"Written: {out_path}")

    # Summary
    print(f"\n=== SUMMARY ===")
    print(f"Yogurt: {len(yogurt_raw)} raw records")
