...
def gate_coverage(frontend, corpus):
    ...
    if schema_ver == "v3":
        ...
        if not copy_stage_ran:
            ...
            # An ABSENT field is not "pending": it means this page does not render the
            # deep-dive block (golden/assessment shape: brined-cheeses, crackers, yogurt).
            # Only a literal PENDING_COPY placeholder is a failure -- the semantics
            # consumerTakeaway and bariInterpretation already use. Counting absence as
            # pending failed the already-shipped crackers page.
            def _ce(p):
            ...
            # bestUseCases
            # same rule: empty/absent list == page does not render the tag row
            buc_pending = sum(
                1 for p in products
