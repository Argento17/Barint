...
| **co_sign** | Nutrition D7 — **co-signed (TASK-181F, 2026-06-04)**: confidence criteria (bound_value_set_1, incl. D5-band dependency + two-signal fallback), confidence_scale 1.0/0.70/0.40 + the `50+(base−50)×scale` formula (bound_value_set_2), and population_correlation 0.05/0.15/0.40/0.75 (bound_value_set_3) are all scientifically defensible and bound as authored. Section 3 consumer framing was separately **Product-co-signed 2026-06-04** (spec L427–482) and is not re-opened here. **Product D7 — CO-SIGNED (TASK-181F, 2026-06-04).** Product accepts all three numeric bindings as the right rule to build. Rationale: (1) The de-circularizing move — keying confidence to ingredient-evidence quality, NOT the NOVA class — is the load-bearing product fix: it stops D3 from asserting a confident per-product health verdict the cited population-level cohorts (Monteiro 2019 / Srour 2020 / IARC 2020) do not support. That is a trust/honesty gain, not technical elegance. (2) `low = 0.40` accepted, not too aggressive: the `50+(base−50)×scale` formula pulls only TOWARD neutral 50, never toward a worse score, so a low-confidence NOVA-4 moves 35→44 (less punitive); 0.40 (not 0.0) is the correct floor — 0.0 would erase a weak-but-genuine prior and over-correct a low-confidence UPF to a clean neutral. Routing residual uncertainty to D6 rather than burying it in D3 is the right separation. (3) Committing without consumer-demand data is acceptable HERE because this co-sign binds the rule on paper only (flag default OFF, byte-identical, fully reversible); the bypassed TASK-179X engagement-gate debt is explicitly carried to the separate owner go-live gate (tripwire #1) and is NOT discharged by this signature. (4) Frozen-invariant safety claim accepted: milk run_005_headpin (NOVA 1–2 / high-conf → scale 1.0 → identical), snack 70/B (held by independent sugar/calorie guardrails; D3 weight 0.15 pull-to-neutral cannot promote a bar to A), bread low-conf → pull-to-neutral with no new published run; and flag OFF = byte-identical so nothing moves at co-sign. **This co-sign approves the rule to be built; it does NOT authorize the live flag-flip, which remains a separate owner go-live decision.** Both D7 co-signs now recorded per scoring-rule governance. |
...
| **status** | Proposed-behind-flag (`BARI_GLASSBOX_W4`, default OFF; OFF = byte-identical; not live-active). **Both D7 co-signs complete (Nutrition + Product, TASK-181F, 2026-06-04).** Rule adopted on paper; TASK-181G Data build UNBLOCKED. Live grade movement additionally requires a separate owner go-live decision (carries the bypassed TASK-179X engagement-gate debt). |
