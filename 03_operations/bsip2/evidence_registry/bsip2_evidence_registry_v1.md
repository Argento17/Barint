...
| **should_affect_score_now** | true — all three scoring gates CLOSED (see scoring_milestone); wired into glycemic_quality and satiety_support as capped presence-only bonus. See `run_ev006_regression.py` for 40/40 regression tests. |
...
| **vocabulary_milestone** | ✅ **CLOSED PASS (2026-06-10)** — viscous/non-viscous fiber vocabulary dictionary completed. Artifact: `01_framework/bsip2_framework/docs/scoring/fiber_functional_vocabulary_v1.md` (FFV-v1), covering all 12 target fibers with Hebrew + English patterns, E-numbers, functional class, false-positive risks, Israeli-label context. Dictionary was documented-only, imported nowhere at this stage. |
| **scoring_milestone** | ✅ **CLOSED PASS (2026-06-10)** — all three scoring gates resolved. (1) Presence-only credit cap: `FIBER_FUNCTIONAL_BONUS.presence_bonus_cap_per_dimension = 2`. (2) Viscous vs non-viscous weighting: viscous gets +2 (glycemic) / +2 (satiety), non-viscous prebiotic gets +1 / +1. Both capped at 2 per dimension. (3) Regression fixtures: `run_ev006_regression.py` — 12 signal fixtures + 24 dimension tests + 4 zero-drift checks = 40/40 PASS. |
