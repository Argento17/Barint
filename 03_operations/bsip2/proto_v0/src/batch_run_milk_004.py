"""
BSIP2 Prototype v0 — Milk & Alternatives Batch Runner (run_004_recalibrated)
Source: same BSIP1 corpus as run_002/003 (Yohananof real retailer scrape)
Recalibration applied (v2 grade philosophy):
  - Grade thresholds: S≥90, A≥80, B≥65, C≥50, D≥35, E<35
  - NOVA_PROCESSING_SCORES: {1:95, 2:85, 3:65, 4:35}
  - NOVA_WFI_SCORES: {1:100, 2:85, 3:60, 4:30}
  - NOVA1_SINGLE_FLOOR: 75→85
  - WHOLE_FOOD_FAT_FLOOR: 65→70
  - NOVA4 cap: 60→68, NOVA3 cap: 75→82
  - ADDITIVE_3PLUS cap: 65→72, ADDITIVE_5PLUS cap: 55→60
  - CONFIDENCE_LOW_CEILING: 70→75
Output: 02_products/milk_and_alternatives/intelligence_bsip2/run_004_recalibrated/
"""
import sys
import json
import pathlib
...
OUTPUT_ROOT  = pathlib.Path(r"C:\Bari\02_products\milk_and_alternatives\intelligence_bsip2\run_004_recalibrated")
REPORT_ROOT  = pathlib.Path(r"C:\Bari\02_products\milk_and_alternatives\intelligence_bsip2\run_004_recalibrated\reports")
...
def run_batch():
    log.info("=== BSIP2 Milk & Alternatives — run_004_recalibrated (v2 grade recalibration) ===")
...
def _write_summary(traces, errors):
    ...
    lines = [
        f"# BSIP2 Milk & Alternatives — {RUN_ID} Summary",
        f"\n**Run date:** {run_dt}",
        f"**Category:** {CATEGORY_TAG}",
        f"**Source:** Yohananof — real retailer scrape (same corpus as run_002/003)",
        f"**Recalibration:** v2 grade philosophy (S/A/B/C/D/E thresholds, NOVA smoothing, floor/cap lift)",
