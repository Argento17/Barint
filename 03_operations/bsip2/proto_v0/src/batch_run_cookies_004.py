# BSIP2 batch -- Cookies-near-coffee (run_cookies_004) / TASK-275/P89
# P89 remediation run: corpus 59 (dropped ck-7290106656727 + ck-7290119043149),
# re-routed ck-80083764 to biscuit (EV-058), RT-6 E-number fix in gen_frontend_json.
# Mirrors batch_run_cookies_003.py structure. OFF ban: absolute.
...
BSIP2_OUTPUT= ROOT/"02_products"/"cookies_coffee"/"bsip2_outputs"/"run_cookies_004"
RUN_ID = "run_cookies_004"
EXPECTED_COUNT = 58  # corpus 61 - 3 dropped to OUT_OF_SCOPE (P89: 2 content drops + 1 EV-058 routing fallback)
...
def main():
    ...
    log.info("=== BSIP2 Cookies-near-coffee -- %s (P89 remediation) ===", RUN_ID)
    ...
    log.info("P89 changes: corpus 59 (dropped 2 OOS), EV-058 oat anchor, RT-6 E-number fix")
    ...
    if len(bsip1_records) != EXPECTED_COUNT:
        log.warning("Expected %d BSIP1 records, got %d", EXPECTED_COUNT, len(bsip1_records))

    traces, score_errors, brined_flag_fired, routing_cats = [], [], [], {}

    # Track re-routed oat product specifically
    OAT_BARCODE = "80083764"
    oat_trace_summary = None
    ...
    for doc in bsip1_records:
        ...
        try:
            ...
            if barcode == OAT_BARCODE:
                oat_trace_summary = {"barcode":barcode,"name":name,"score":score,"grade":grade,
                                     "category":cat,"nova":nova,
                                     "binding_cap":trace.get("binding_cap"),
                                     "drivers":extract_drivers(trace)}
                log.info("  OAT_PRODUCT [EV-058]: %-40s score=%-5s grade=%-2s cat=%-20s binding_cap=%s",
                         name[:38], score, grade, cat, trace.get("binding_cap"))
    ...
    with verify_path.open("w", encoding="utf-8", newline="") as vf:
        vf.write("barcode,score,grade,binding_cap,nova,fat_g,sat_fat_g,sugar_g,sodium_mg,context_flag,category" + "\n")
    ...
    run_record = {
        "run_id":RUN_ID,"task":"TASK-275_P89","category_slug":"cookies-coffee",
        "generated":ts,"engine":"proto_v0 / score_engine.py (unmodified)",
        "p89_changes":{
            "corpus_drops":["ck-7290106656727 (children Elite, RT-4/P88)","ck-7290119043149 (לה פזואלוס butter, extraction failure)"],
            "router_change":"EV-058: added 'עוגיות שיבולת שועל' anchor (conf=0.86) to route oat biscuits to biscuit category",
            "rt6_fix":"gen_frontend_json.py: E-number regex fix (strip scraper suffix) + E200/E160A added to ADDITIVE_DB",
        },
        ...
        "oat_product_ev058":oat_trace_summary,
        "router_ev":"EV-058",
    ...
    print("\n" + "="*70)
    print(f"COOKIES-NEAR-COFFEE BSIP2 RUN -- {RUN_ID} (P89 REMEDIATION)")
    print("="*70)
    ...
    if oat_trace_summary:
        print(f"OAT PRODUCT EV-058 [ck-{OAT_BARCODE}]:")
        print(f"  score={oat_trace_summary['score']}  grade={oat_trace_summary['grade']}  cat={oat_trace_summary['category']}  binding_cap={oat_trace_summary['binding_cap']}")
    ...
    if brined_flag_fired:
        for p in brined_flag_fired: print(f"  CRITICAL: {p['barcode']} {p['name']}")
    print("TOP 3:")
    for i, p in enumerate(top3,1):
        print(f"  {i}. [{p['score']}/{p['grade']}] {str(p.get('name','?'))[:50]} cat={p.get('category')} nova={p['nova']}")
    print("BOTTOM 3:")
