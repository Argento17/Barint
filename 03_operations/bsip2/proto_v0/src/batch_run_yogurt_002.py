"""
BSIP2 Prototype v0 — Yogurt Batch Runner (run_yogurt_002)
Source: REAL Israeli yogurt SKUs from Open Food Facts (BSIP0 run_yogurt_002).
Framework: proto_v0 / engine 0.4.0 (UNMODIFIED — no manual score edits).
Purpose: produce reproducible machine scores for the real OFF yogurt corpus so
         TASK-135 can reconcile vs the displayed manual-MVP shelf and decide on
         retiring DEC-005. NOTE: the OFF corpus is ingredient-blind (0 ingredient
         lists), so this run measures exactly how ingredient-blindness degrades
         yogurt scoring confidence — it is evidence, not a shippable shelf.
Output: 02_products/yogurt_system/bsip2_outputs/run_yogurt_002/
"""
...
BSIP1_SOURCE = pathlib.Path(r"C:\Bari\03_operations\bsip1\run_yogurt_002\output")
OUTPUT_ROOT  = pathlib.Path(r"C:\Bari\02_products\yogurt_system\bsip2_outputs\run_yogurt_002")
...
def run_batch():
    log.info("=== BSIP2 Yogurt — %s (REAL OFF corpus) ===", RUN_ID)
    ...
    for product in products:
        ...
        try:
            ...
            traces.append(trace)
            log.info("  %-22s score=%-5s grade=%-3s cat=%-16s nova=%s suf=%s",
    ...
    summary = {
        ...
        "source": "Open Food Facts — real Israeli yogurt SKUs (ingredient-blind)",
