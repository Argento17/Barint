"""
BSIP2 Prototype v0 — Yogurt Batch Runner (run_yogurt_003)
Source: REAL Israeli yogurt SKUs from SHUFERSAL product pages (BSIP0 run_yogurt_003)
        — WITH ingredient panels + Hebrew names + nutrition (unlike OFF run_yogurt_002).
Framework: proto_v0 / engine 0.4.0 (UNMODIFIED — no manual score edits).
Purpose: produce reproducible machine scores on a real, ingredient-BEARING yogurt
         corpus so TASK-135 can decide whether DEC-005 (manual-MVP shelf) can retire.
Output: 02_products/yogurt_system/bsip2_outputs/run_yogurt_003/
"""
...
BSIP1_SOURCE = pathlib.Path(r"C:\Bari\03_operations\bsip1\run_yogurt_003\output")
OUTPUT_ROOT  = pathlib.Path(r"C:\Bari\02_products\yogurt_system\bsip2_outputs\run_yogurt_003")
REPORT_ROOT  = pathlib.Path(r"C:\Bari\02_products\yogurt_system\reports")
RUN_ID       = "run_yogurt_003"
...
def run_batch():
    log.info("=== BSIP2 Yogurt — %s (REAL Shufersal corpus, ingredient-bearing) ===", RUN_ID)
    ...
    for product in products:
        ...
        try:
            ...
            traces.append(trace)
            log.info("  %-26s score=%-5s grade=%-4s cat=%-14s nova=%s suf=%s",
    ...
    summary = {
        ...
        "source": "Shufersal — real Israeli yogurt SKUs (ingredient-bearing)",
