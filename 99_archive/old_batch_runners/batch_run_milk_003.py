"""
BSIP2 Prototype v0 — Milk & Alternatives Batch Runner (run_003)
Source: same BSIP1 corpus as run_002 (Yohananof real retailer scrape)
Fixes applied vs run_002:
  - Fix 1: Beverage liquid gate expanded (nutrition_basis_claimed, brand, ingredient heuristics)
  - Fix 2: SE_BEVERAGE_KCAL lowered 20→10 (plain dilute plant milks exempt from SE)
Output: 02_products/milk_and_alternatives/intelligence_bsip2/run_003/
"""
import sys
import json
import pathlib
...
OUTPUT_ROOT  = pathlib.Path(r"C:\Bari\02_products\milk_and_alternatives\intelligence_bsip2\run_003")
REPORT_ROOT  = pathlib.Path(r"C:\Bari\02_products\milk_and_alternatives\intelligence_bsip2\run_003\reports")
...
def run_batch():
    log.info("=== BSIP2 Milk & Alternatives — run_003 (fixes: F1 beverage gate + F2 SE threshold) ===")
...
def _write_summary(traces, errors):
    ...
    lines = [
        ...
        f"**Source:** Yohananof — real retailer scrape (same corpus as run_002)",
        f"**Fixes applied:** F1 (beverage liquid gate expansion) + F2 (SE_BEVERAGE_KCAL 20→10)",
