...
|---|----------|----------------------|-----------|-------------------|
| **1** | **Frozen invariant / ~~published scores~~ / scoring philosophy** | Constitutional — rewrites the product's truth | Re-opening milk = 85/A; lifting the snack-bar B ceiling; redesigning BSIP2 weights; changing bread provenance | Re-scoring a category *behind a flag*; a recal that **holds** the invariant; methodology drafts; **score MOVEMENTS from verified data corrections with `\|Δ\|≤30`pts (owner ruling 2026-07-11) — orchestrator applies autonomously through the existing engine; `\|Δ\|>30` = DEFECT, investigate not ship; deploy still owner-merge** |
