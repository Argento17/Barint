/* plant.js — Bari sprout: assemble → breathe → shatter-into-constellation → reform.
   Drives 5 pixel-sliced logo pieces (full-frame, so they stack into the exact logo).
   Vanilla, RAF-driven. Configure via window.plant.setConfig({...}). */
(function () {
  "use strict";
  ...
  PlantHero.prototype.setConfig = function (partial) {
    ...
    if (partial.size !== undefined) { /* handled via var */ }
  };
  ...
  PlantHero.prototype._tick = function (now) {
    ...
    for (var i = 0; i < PIECES.length; i++) {
      ...
      var ny = cen[1] * 100 * ASPECT + (ty / size) * 100; // svg viewBox y is *ASPECT scaled
    ...
    var vis = (this._mode === "idle") ? shatterD : (1 - clamp((t)/1.0,0,1)*0 ); // show during shatter
