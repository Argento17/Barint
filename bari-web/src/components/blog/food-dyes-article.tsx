...
import { FoodDyesArticleHero } from "@/components/blog/food-dyes-article-hero";
  ...
  return (
    <li className="rounded-[1rem] border border-black/[0.07] bg-[#FFFFFF] px-5 py-5">
      ...
      <p className="mt-2 text-xs leading-relaxed text-[#7A817C]">
        <span className="font-bold">בפועל: </span>
        {availability}
      </p>
    </li>
  );
...
function ScienceSection({ science }: { science: typeof foodDyesArticle.science }) {
  const reduceMotion = useReducedMotion();
  return (
      ...
      <HomeContainer>
        <div className="mx-auto max-w-3xl">
          <header className="mb-8 text-right">
            ...
            <p className="mt-3 text-base leading-relaxed text-[#4E5663]">{science.subtitle}</p>
          </header>
            ...
            <ol className="space-y-3">
              {science.citations.map((c) => (
                <li key={c.id} className="flex gap-3 text-right">
                  ...
                  <div>
                    <p className="text-xs font-semibold leading-relaxed text-[#111318]">{c.short}</p>
                    <p className="mt-0.5 text-[0.65rem] leading-relaxed text-[#7A817C]">{c.claim}</p>
                    <a
                      href={c.url}
                      target="_blank"
...
export function FoodDyesArticle() {
  const article = foodDyesArticle;
  ...
  return (
    <div className={cn("bg-[#F7F7F2] text-[#111318]", siteHeaderOffsetClass)}>
      <article>
        ...
        <section id="regulatory-gap" className="border-t border-black/6 bg-[#F7F7F2] py-14 md:py-20">
          <HomeContainer>
            <div className="mx-auto max-w-3xl">
              <header className="mb-8 text-right">
                <p className="font-mono text-[0.65rem] font-bold uppercase tracking-[0.24em] text-[#7A9450]/85">
                ...
                <h2 className="mt-2 text-3xl font-extrabold tracking-tighter text-[#111318] md:text-4xl">
                  {article.regulatoryGap.title}
                </h2>
                <p className="mt-3 text-base leading-relaxed text-[#4E5663]">
                  {article.regulatoryGap.subtitle}
                </p>
              </header>
              <div className="space-y-5">
                {article.regulatoryGap.paragraphs.map((p, i) => (
        ...
        {/* Science of the dye-attention evidence + citations */}
        ...
        <HomeContainer className="pb-4">
          <div className="mx-auto max-w-3xl">
            <InsightBlock quote={article.editorialInsights[2]} index={1} />
          </div>
        </HomeContainer>
