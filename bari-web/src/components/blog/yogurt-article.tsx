...
import { YogurtArticleHero } from "@/components/blog/yogurt-article-hero";
...
import {
  ...
  HASHVAOT_HREF,
} from "@/lib/blog/yogurt-article-content";
import { siteHeaderOffsetClass } from "@/lib/site-layout";
import { cn } from "@/lib/utils";

/* ──────────────────────────────────────────────
   Shared inline sub-components
   (second article using these patterns — extraction
   to src/components/blog/shared/ is now warranted per spec)
────────────────────────────────────────────── */
...
function ScienceSection({
  ...
  science: typeof yogurtArticle.science;
}) {
  const reduceMotion = useReducedMotion();
  return (
...
function YogurtExternalResearch() {
  const { externalResearch } = yogurtArticle;
  return (
    <div className="bg-[#F7F7F2] py-14 md:py-20">
      <HomeContainer>
        <div className="mx-auto max-w-3xl">
          ...
          <div className="space-y-5">
            {externalResearch.keyFindings.map((finding) => (
              <div
                key={finding.source}
                className="rounded-[1.1rem] border border-black/[0.07] bg-[#FFFFFF] p-6"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 text-right">
                    ...
                    <div className="mt-2 flex items-baseline gap-2">
                      <p className="text-3xl font-extrabold tracking-tight text-[#7A9450]">
                        {finding.stat}
                      </p>
...
/* ──────────────────────────────────────────────
   Root article component
────────────────────────────────────────────── */

export function YogurtArticle() {
  ...
  return (
    <main className={cn("bg-[#F7F7F2] text-[#111318]", siteHeaderOffsetClass)}>
      <article>
        ...
        {/* Science — fermentation, protein, live cultures, sugar */}
        <ScienceSection science={article.science} />

        {/* What 'yogurt' guarantees — and what it doesn't */}
        <section
          id="yogurt-grade"
          ...
          <HomeContainer>
            <div className="mx-auto max-w-3xl">
              <header className="mb-8 text-right">
                ...
                <h2 className="mt-2 text-3xl font-extrabold tracking-tighter text-[#111318] md:text-4xl">
                  {article.yogurtGradeGuarantees.title}
                </h2>
                <p className="mt-3 text-base leading-relaxed text-[#4E5663]">
                  {article.yogurtGradeGuarantees.subtitle}
                </p>
              </header>
              <div className="space-y-5">
                {article.yogurtGradeGuarantees.paragraphs.map((p, i) => (
                  <p
                    key={i}
                    className="text-base leading-[1.85] text-[#111318] md:text-lg"
                  >
        ...
        {/* Plain vs flavored comparison matrix */}
        <YogurtComparisonMatrix />
        ...
        <HomeContainer className="space-y-16 py-8 md:space-y-24 md:py-12">
          <div className="mx-auto max-w-4xl space-y-16 md:space-y-24">
            ...
            <aside className="rounded-[1.1rem] border border-[#7A9450]/20 bg-[#FFFFFF] p-6 text-right">
              <p className="font-mono text-[0.65rem] font-bold uppercase tracking-[0.18em] text-[#7A9450]/85">
                השוואה אינטראקטיבית
              </p>
              <h3 className="mt-2 text-lg font-extrabold tracking-[-0.02em] text-[#111318]">
                רוצים להשוות את כל 19 היוגורטים?
              </h3>
              <p className="mt-1.5 text-sm leading-relaxed text-[#4E5663]">
                דף ההשוואה מציג את כל המוצרים עם ציון, דרגה, ונתוני תווית — לפי סינון שבוחרים.
              </p>
              <Link
                href={YOGURT_HASHVAOT_HREF}
                className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#7A9450] px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-[transform,box-shadow] duration-300 hover:-translate-y-0.5"
              >
                לדף השוואת יוגורטים
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div className="max-w-xl space-y-2 text-right">
                  ...
                  <p className="text-sm leading-relaxed text-[#C8CDC9] md:text-base">
                    דגנים, גבינות, חמאה, לחם, חומוס ועוד — כל קטגוריה עם ניתוח מלא ומנוע השוואה אינטראקטיבי.
                  </p>
                </div>
                <Link
                  href={HASHVAOT_HREF}
