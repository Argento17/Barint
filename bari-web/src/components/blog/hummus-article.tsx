...
import { HummusArticleHero } from "@/components/blog/hummus-article-hero";
...
import {
  ...
  HASHVAOT_HREF,
} from "@/lib/blog/hummus-article-content";
...
function CategoryBoundarySection() {
  const { categoryBoundary } = hummusArticle;
  return (
    <section
      ...
      <HomeContainer>
        <div className="mx-auto max-w-3xl">
          <header className="mb-8 text-right">
            ...
            <h2 className="mt-2 text-3xl font-extrabold tracking-tighter text-[#111318] md:text-4xl">
              {categoryBoundary.title}
            </h2>
            <p className="mt-3 text-base leading-relaxed text-[#4E5663]">
              {categoryBoundary.subtitle}
            </p>
          </header>
          <div className="space-y-5">
            {categoryBoundary.paragraphs.map((p, i) => (
              <p
                key={i}
                className="text-base leading-[1.85] text-[#111318] md:text-lg"
              >
...
function ExternalResearchSection() {
  const { externalResearch } = hummusArticle;
  const { mainStudy } = externalResearch;
  return (
    <div className="bg-[#F7F7F2] py-14 md:py-20">
      <HomeContainer>
        <div className="mx-auto max-w-3xl">
          ...
          <div className="rounded-[1.2rem] border border-black/[0.07] bg-[#FFFFFF] px-6 py-6 md:px-8 md:py-7">
            ...
            <p className="text-base font-semibold text-[#111318]">
              {mainStudy.headlineLabel}
            </p>
            <p className="mt-4 text-base leading-[1.8] text-[#111318]">
              {mainStudy.description}
            </p>
            <blockquote className="mt-5 border-r-4 border-[#7A9450] pr-4 text-right">
              <p className="text-sm font-semibold italic leading-relaxed text-[#4E5663]">
                {mainStudy.quote}
              </p>
              <footer className="mt-1 text-xs text-[#7A817C]">
                {mainStudy.quoteAttribution}
              </footer>
            </blockquote>
            <p className="mt-5 rounded-[0.7rem] bg-[#F7F7F2] px-4 py-3 text-xs leading-relaxed text-[#7A817C]">
              <span className="font-bold">הסתייגות: </span>
              {mainStudy.caveat}
            </p>
            <a
  ...
  return (
    <main className={cn("bg-[#F7F7F2] text-[#111318]", siteHeaderOffsetClass)}>
      <article>
        ...
        {/* Science / nutritional background */}
        <ScienceSection />

        {/* Category boundary — raw chickpeas vs. prepared spread */}
        <CategoryBoundarySection />
        ...
        {/* Findings */}
        <section id="findings" className="border-y border-black/6 bg-[#FFFFFF] py-14 md:py-20">
          <HomeContainer>
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div className="max-w-xl space-y-2 text-right">
                  <h3 className="text-xl font-extrabold tracking-[-0.03em] text-[#F7F7F2] md:text-2xl">
                    רוצים לראות את ניתוח החומוס המלא?
                  </h3>
                  <p className="text-sm leading-relaxed text-[#C8CDC9] md:text-base">
                    64 מוצרים, פירוט ציונים, רשימות רכיבים — כלי ההשוואה האינטראקטיבי של Bari.
                  </p>
                </div>
                <Link
