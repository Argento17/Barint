...
export function FoodDyesArticleHero() {
  const { hero, disclaimer, heroStats } = foodDyesArticle;
                ...
                <p
                  className={
                    i === heroStats.length - 1
                      ? "text-2xl font-extrabold tracking-tight text-[#C0392B]"
                      : "text-2xl font-extrabold tracking-tight text-[#111318]"
                  }
                >
                  {stat.value}
                </p>
                <p className="mt-0.5 max-w-[12rem] text-xs font-bold uppercase tracking-[0.12em] text-[#7A817C]">
                  {stat.label}
                </p>
              </div>
            ))}
