...
export const chocolateTabletsHero = {
  ...
  title: "השם על טבלת השוקולד המריר מספר פחות מההרכב שבפנים; הציון קורא את הקקאו האמיתי מול הסוכר שלובש את השם",
} as const;

export const chocolateTabletsPrologueSentences = [
  "השם על טבלת השוקולד המריר מספר פחות מההרכב שבפנים: גם טבלת 70% יכולה להיות כמעט שליש סוכר, והמדף מוכיח שצריך לבדוק קקאו מול סוכר יחד.",
...
export const chocolateTabletsCategoryNote = [
  "רק שתי טבלאות במדף הזה מגיעות ל-B, ואחריהן פער של עשר נקודות עד הבאה בתור. שוקולד הוא ממתק צפוף בשומן וסוכר, וגם שתי הטבלאות שמובילות את המדף נושאות את זה במלואו. ה-B מסמן את הצד הטוב של מדף הממתקים, ולא שהמוצר הוא מוצר בריאות.",
