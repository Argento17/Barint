...
export const snackHeroLine =
  "קונים חטיפים בסופרמרקט? הנה מה שאתם צריכים לדעת.";

export const snackHero = {
  eyebrow: "חטיפים",
  title: "חטיפים: 18 מוצרים, הציון הגבוה ביותר עוצר ב-B",
} as const;

export const snackPrologueSentences = [
  "אתם עומדים מול מדף החטיפים. חטיפי תמרים ליד גרנולה, גרנולה ליד פיטנס, פיטנס ליד פרוטאין — כולם על אותו מדף, עם אריזות שמדברות על בריאות.",
  "מה שגילינו הוא שהפערים בין המוצרים גדולים בצורה שמפתיעה.",
  "יש כאן חטיפים עם שלושה רכיבים בלבד — ויש כאן 24 רכיבים עם שלושה מקורות סוכר.",
  "הציון הגבוה ביותר שמצאנו — B — הלך לחטיף תמרים ושקדים, לא לאף אחד מהשמות הגדולים.",
  "הפערים בין מוצרים על אותו מדף מגיעים לעשרות נקודות. ממדף יוחננוף בלבד — לא סקר שוק ישראלי.",
] as const;

export const snackCategoryNote = [
  "הערת קטגוריה — הניקוד הגבוה ביותר הוא B, לא A\n\nבמדף החטיפים אף מוצר לא הגיע ל-A. הציון הגבוה הוא 70/B — והוא שייך לחטיף עם שלושה רכיבים בלבד. זו לא החמרה; זה מה שקטגוריה שנועדת לטעם נמצאת בה. הציון משקף את ההרכב, לא את השם על האריזה.",
  "הערת קטגוריה — ההשוואה היא בתוך המדף, לא מחוץ לו\n\nחטיף B לא שקול לארוחה B ולא לחטיף בריאות. הציון אומר: בתוך המדף הזה, זה מה שעומד בהשוואה. מדד אחד — מי פשוט יותר, מי עמוס יותר, היכן הסוכר מגיע ומאיפה.",
].join("\n\n");

export const snackMethodologyText =
  "בדקנו 53 חטיפים ממדף יוחננוף — תמרים, גרנולה, דגנים מצופים ופרוטאין. 48 מוצרים קיבלו ציון; 18 נבחרו לתצוגה על בסיס מגוון קטגוריות ופערי ציון משמעותיים. הציונים יחסיים לקטגוריה; הציון הגבוה ביותר שמצאנו — B — אינו מגיע ל-A. המדידה: בסיס מבני, מקורות סוכר, עומס עיבוד ורמת תוספים — לא קלוריות בלבד.";
