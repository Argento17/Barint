...
export const brinedCheesesPrologueSentences = [
  ...
  "הנדיר כאן הוא המינימליזם: רק שלוש גבינות במדף מסתפקות בגרעין עצמו — חלב, מלח, תרבית ומקריש. המרשימה שבהן היא הבולגרית 13% של יורו מחלבות אירופה: רשימה נקייה, תרבית לקטית, נתרן נמוך יחסית של 720 מ\"ג, וציון בדירוג A. וכאן מתחדדת הנקודה: ראש המדף שייך דווקא לצפתיות של מחלבות גד, גבינות מצוינות שנשענות על חומר משמר. וטמרה של רג'ב, מהנקיות במדף, יושבת דווקא ב-C כי הנתרן שלה הוא הגבוה מכולן. רשימת רכיבים נקייה לא תמיד אומרת ציון הכי גבוה, וזה בדיוק מה שבארי באה למדוד.",
] as const;
