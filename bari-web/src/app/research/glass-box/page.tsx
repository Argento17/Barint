...
export const metadata: Metadata = {
  title: "איך בארי בונה את הציון | Bari",
  description:
    "בארי תיקנה הנחה שמוצרים שנראים פחות מעובדים מוערכים בצורה אחרת. הסבר על השינוי ומה בארי בודקת עכשיו.",
};
      ...
      <div className="mx-auto w-full max-w-[680px] overflow-hidden bg-white sm:rounded-[1.5rem] sm:shadow-xl">
        ...
        <section className="px-4 pt-5 pb-4 sm:px-6">
          ...
          <h1 className="mt-1 text-[1.35rem] font-semibold leading-tight tracking-[-0.028em] text-[#111318]">
            איך בארי בונה את הציון
          </h1>
          <p className={`mt-3 ${BODY_TEXT}`}>
            בארי עצרה את ההנחה שמזון שנראה פחות מעובד אוטומטית פחות בעייתי.
          </p>
          <p className={`mt-2 ${BODY_TEXT}`}>
            זו לא שאלה של נתונים טובים יותר — זו שאלה של עקביות. אם שילוב של שומן עם מלח, או שומן עם סוכר, סופר בצורה אחת בגלידה וסופר אחרת בחומוס — הציון משקר. בארי תיקנה את זה.
          </p>
        </section>
        ...
        <section className={`${SECTION_BORDER} px-4 pt-4 pb-4 sm:px-6`}>
          <p className={BODY_TEXT}>
            שילוב של שומן עם מלח — ושילוב של שומן עם סוכר — קשור לקלות שבה אוכלים יותר מדי מאותו מוצר. בארי ספרה את השילוב הזה תמיד, אבל לא תמיד באותו משקל: מוצר שנראה פחות מעובד קיבל הנחה.
          </p>
          <p className={`mt-3 ${BODY_TEXT}`}>
            זו הייתה טעות. לא טעות בנתונים — טעות בעיקרון. המראה של מוצר אינו מקטין את ההשפעה של ההרכב שלו על הצלחת. בארי מפסיקה להתחשב במראה: אם התווית מראה את השילוב, הוא נספר באותו משקל בכל מוצר. מוצרים שנהנו מהנחה הזו ירדו בציון.
          </p>
        </section>

        {/* ── Section 3: מה בארי בודקת עכשיו ──────────────────────────────── */}
        ...
        <section className={`${SECTION_BORDER} px-4 pt-4 pb-3 sm:px-6`}>
          <div className="space-y-4">
            <p className={BODY_TEXT}>
              ...
              <br />
              האם המוצר מגלה את כל רכיביו? בארי בודקת עד כמה התווית שקופה — שמות גנריים של תוספים, חומרים שלא מפורטים, מידע חסר.
            </p>
            <p className={BODY_TEXT}>
              <strong className="font-semibold text-[14px] text-[#2F3531]">כמה בטוחים אנחנו</strong>
              <br />
              לא כל מוצר נבדק באותה רמת ודאות. בארי מציגה את רמת הביטחון של כל ציון, ומאזנת אותה כלפי ניטרלי כשהמידע חלקי.
            </p>
            <p className={BODY_TEXT}>
              <strong className="font-semibold text-[14px] text-[#2F3531]">אילו תוספים נמצאים ומה ידוע עליהם</strong>
              <br />
              בארי מציגה את רשימת התוספים שבמוצר עם הסבר קצר על כל אחד — מה תפקידו, ומה המחקר אומר עליו עד היום. זה מידע להבנה, לא פסיקה. הרשימה הזו לא משנה כרגע את הציון הראשי.
            </p>
            <p className={BODY_TEXT}>
              <strong className="font-semibold text-[14px] text-[#2F3531]">אות עיבוד — בהתאם לביטחון</strong>
              <br />
              בארי בוחנת את דפוס הרכיבים של המוצר ומשווה אותו לנתונים ממחקרים גדולים על אוכלוסיות. כשהמידע על הרכב המוצר שלם, האות חזק. כשהמידע חלקי, האות נסוג לניטרלי — בארי לא ממציאה ביטחון שאין.
            </p>
          </div>
        </section>
