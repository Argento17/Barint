import type { Metadata } from "next";

import { BlogArticleSeo } from "@/components/seo/blog-article-seo";
import { absoluteUrl } from "@/lib/site-url";
import { OfPoultryReportArticle } from "@/components/news/of-poultry-report-article";
import { seoMeta } from "@/lib/news/of-poultry-report-content";

export const metadata: Metadata = {
  title: seoMeta.title,
  description: seoMeta.description,
  openGraph: {
    title: seoMeta.ogTitle,
    description: seoMeta.ogDescription,
    type: "article",
  },
};

export default function OfPoultryReportNewsPage() {
  return (
    <>
      <BlogArticleSeo
        title={metadata.title as string}
        description={metadata.description as string}
        url={absoluteUrl("/news/of-poultry-report")}
      />
      <OfPoultryReportArticle />
    </>
  );
}
