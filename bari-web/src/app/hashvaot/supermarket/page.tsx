...
export default function SupermarketCategoryPage() {
  ...
  const granolaSorted = [...granolaProducts].sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
  const granolaTop = granolaSorted[0];
  const granolaBottom = granolaSorted[granolaSorted.length - 1];
  const granolaSpread = Math.round((granolaTop?.score ?? 0) - (granolaBottom?.score ?? 0));
  const granolaDescription = `גרנולה ומוזלי נראים כמו בחירת הבריאות של המדף — אבל ${granolaProducts.length} המוצרים שבדקנו נעים בין ${Math.round(granolaTop?.score ?? 0)}/${granolaTop?.grade ?? ""} ל-${Math.round(granolaBottom?.score ?? 0)}/${granolaBottom?.grade ?? ""}, פער של ${granolaSpread} נקודות. הציון תלוי בכמות הסוכר, השומן והסירופ בפועל, לא בתדמית.`;
  ...
  const proteinBarsDescription = `דוח השוואה לחטיפי החלבון: ${proteinBarsProducts.length} חטיפים עם 25–36 גרם חלבון ל-100 גרם — והמחיר ההנדסי שמגיע איתם. קטגוריה נפרדת מחטיפי הדגנים.`;
  const chocolateTabletsDescription = `בדקנו ${chocolateTabletsProducts.length} טבלאות שוקולד: מריר 90% עד שוקולד לבן ממולא. הציון הגבוה ביותר הוא B, ורק שתי טבלאות מגיעות אליו — כל השוקולד הוא ממתק, אבל הפער בין 2 גרם סוכר ל-65 גרם הוא אמיתי.`;
  const chocolateBarsDescription = `בדקנו ${chocolateBarsProducts.length} חטיפי שוקולד: כולם ציון E, 27–60 גרם סוכר ל-100 גרם. ההבדל היחידי הוא בין חטיף עם בוטנים לחטיף שהוא רק סוכר ושמן מתחת לציפוי.`;
