"""
BSIP2 Prototype v0 — Yogurt System Batch Runner (run_yogurt_001)
Source: Yohananof — 45 canonical products across 13 subtypes
Framework: v2 grade calibration (unmodified proto_v0)
Purpose: stress-test BSIP2 against fermentation, routing ambiguity,
         NOVA1 floor vs red-label cap, vanilla=NOVA4 bug, sat_fat penalty
         on natural full-fat, sweetener caps, plant-based routing failures.
Output: 02_products/yogurt_system/bsip2_outputs/run_yogurt_001/
"""
import sys
import json
import pathlib
...
BSIP1_SOURCE = pathlib.Path(r"C:\Bari\03_operations\bsip1\run_yogurt_001\output")
OUTPUT_ROOT  = pathlib.Path(r"C:\Bari\02_products\yogurt_system\bsip2_outputs\run_yogurt_001")
REPORT_ROOT  = pathlib.Path(r"C:\Bari\02_products\yogurt_system\reports")
CATEGORY_TAG = "yogurt_system"
...
def run_batch():
    log.info("=== BSIP2 Yogurt System — %s ===", RUN_ID)
    ...
    for product in products:
        ...
        try:
            ...
            subtype = product.get("bsip_yogurt_subtype", "?")
            cap     = trace.get("binding_cap") or "-"
            log.info("  %-45s score=%-5s grade=%s  cat=%-18s nova=%s  sub=%-16s cap=%s",
...
def _write_summary(traces, errors):
    ...
    lines = [
        f"# BSIP2 Yogurt System — {RUN_ID} Batch Summary",
        f"\n**Run date:** {run_dt}",
        f"**Category:** {CATEGORY_TAG}",
        f"**Source:** Yohananof — 45 synthetic canonical products (13 subtypes)",
        f"**Framework:** v2 grade calibration (proto_v0 unmodified)",
