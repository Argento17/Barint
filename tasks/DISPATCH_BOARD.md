...
- 🚀 **TASK-568 DISPATCHED** (2026-07-10, frontend-agent "frontend-568") — derived featured cards: Phase 1 scoping doc (data vs copy vs design per card; StoryCard shape; parity-check design) then pilot 2-3 cards in worktree C:/bari_wt_568 → PR. Derivation in lib/ at build time (VM doctrine: UI never sorts/rounds/interprets); approved copy stays literal (FREEZE + two-gate); pixel-identical; every hardcoded-vs-JSON drift discrepancy must be listed in the return.
