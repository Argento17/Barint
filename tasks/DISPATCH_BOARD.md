...
🚀 **TASK-582 DISPATCHED** (2026-07-10 night) — Data Agent (sonnet pin), **Layer-2 fallback, trigger logged in TASK-582.md**: Codex sandbox has no network and the exit criterion is a live 3-barcode Shufersal canary. Scope: fix 01_acquire_shufersal.py URL layer mirroring Shelf Watch's verified `p_{barcode}` pattern; canary to scratch dir only; OFF-ban guard carried.
🚀 **TASK-589 DISPATCHED** — BUILD-LIGHT (Codex gpt-5.6-terra) in worktree C:/bari_wt_589 (branch task589-router-telemetry off 841d0c83): telemetry entry rows + duration_s + best-effort token parse; law tables untouchable (--selftest-table must stay green); no commit per the new sandbox-git rule.
