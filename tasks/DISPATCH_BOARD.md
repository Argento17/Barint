...
- **PHASE 1 → TASK-341 (data-agent, C1-Sonnet in isolated WORKTREE — risk-avoidance per owner "without risks"; cloud-CLI tree-wipe hazard sidestepped) — 🔵 DISPATCHED (bg, 2026-06-19).** Scoped to feasible core: build `integrations/clients/hebrew_grammar_gate.py` (DictaBERT gender/agreement + illegal-form flags) + reader-context lock + tests (must flag a real gender mismatch, pass a clean sentence). PROBE-only (don't build): Dicta-LM 3.0 idiom-reviewer feasibility (NO 24B download), auto-fix loop design, HspellPy AGPL note. No scores/engine/content_voice touch; build standalone, wiring is a reviewed follow-on. Voice work committed b618092c5 first (protection). Verify-on-return: gate catches mismatch + passes clean + feasibility recommendation. Build the Hebrew-correctness layer Bari lacks: DictaBERT gender/agreement gate + spelling/morphology legality; Dicta-LM 3.0 as a native-vs-translated idiom REVIEWER (never the author); reader-context lock (plural אתם/register); auto-fix loop. Wire into the gate chain AFTER voice-match. Verified-real tools: [DictaBERT/Dicta-LM 3.0](https://huggingface.co/dicta-il), [HebPipe](https://github.com/amir-zeldes/HebPipe) (Java, inactive — side-tool only), [HspellPy](https://pypi.org/project/HspellPy) (⚠️AGPL — sandbox only, NOT embedded). Staging/test only, no scores. **Lane: cloud C1 build (Grok/Gemini/Cursor), NOT Sonnet** (spec-complete infra; honors [[native_subagent_pins_sonnet_trap]]).
...
- **PHASE 2+3 → TASK-342 (content-agent + frontend) — 🔴 BLOCKED.** depends_on TASK-341 + locked fingerprint v1.0 + owner's exact Phase-3 structure spec (owner provides "when we get there"). Combined per-page tone+structure rewrite of all 10, copy+structure only, each page staged + owner-gated go-live.
