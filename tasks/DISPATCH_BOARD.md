...
🔧 **TASK-603 DISPATCHED (Vercel gating mechanism)** — ENGINEERING-RESEARCH (Codex terra +web): the project's Deployment Checks uses the `repository-dispatch/actions/status@v1` push flow, NOT auto-observed checks — so the 6 job names never appear to tick (corrects the TASK-571 click-path). Research the exact mechanism (token vs GitHub-app auth, why the list is empty, minimal wiring for 6 jobs across 2 workflows, paid-tier gate) → then implement the workflow steps + give owner the remaining dashboard/secret clicks. First dispatch failed on a driver kwarg (worktree→cwd, audit-E1 class), re-dispatched.
