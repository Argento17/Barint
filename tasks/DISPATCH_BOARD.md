...
🚀 **dispatch.py rebuild DISPATCHED** ("build-583", claude-sonnet-5 = BUILD-HEAVY FALLBACK, trigger "codex OAuth pending" — the router's first dogfooded routing decision): delete grok/cursor/deepseek, repoint opencode HTTP (challenge-GPT / evidence / grunt-text-fallback), codex exec lanes, gemini report-only lane, route() + MODEL_BINDING, --selftest-table asserts code==doc, telemetry JSONL. Two-tree delivery (local + worktree C:/bari_wt_583 → PR).
