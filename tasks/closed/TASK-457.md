...
close_reason: >
  Merged via PR #42 (merge b632f9c6). P462 (Grok, worktree t449-style isolation) wired
  apply_protein_bar_lens behind BARI_PROTEIN_BAR_V1 (49f579d6, score_engine.py ~5244) — ships DORMANT:
  census proved default-flag inertness (eligibility limb pre-existed; 6 cross-page latent subtype hits all
  score-inert). Gate B: flag-ON reproduces the published protein page 32/32 exact (score AND grade) via
  protein_bars_reproduce_harness.py exit 0 — last unreproducible category cured. Gate A: flag-OFF 0 diffs
  cross-corpus. C0 contract PASS. The de-anchor 3-flip activation remains a SEPARATE gated deploy
  (not this task). Router latent hits routed to nutrition backlog.
depends_on: [TASK-456]
blocks: []
