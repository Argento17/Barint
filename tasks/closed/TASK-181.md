...
**Owner chose option 1 (2026-06-04): note + flip ship together.** Opened **TASK-181N** (Content) — authored the dated consumer methodology note (principle framing; names the fat+salt combo). Owner approved the wording in principle.

### W4 GO-LIVE DEFERRED — batched into one GlassBox consumer launch (owner, 2026-06-04)
Owner decided NOT to flip W4 now. Rationale (efficiency + trust): the note and the grade moves are coupled (no silent flip), so deferring the content defers the whole flip — and that's the better call. GlassBox has more consumer-facing changes ahead (later waves / W5); flipping W4 now then moving grades again later = repeated shelf churn a shopper experiences as "why does this keep changing?". One coherent launch (all grade moves + the full consumer narrative + notes together) beats drip-by-drip. Zero cost to waiting: **W4 is built, verified, OFF-byte-identical, and safe — a BANKED capability** (181F–181M all done/closed). 
- **Flip stood down.** `BARI_GLASSBOX_W4` + `NEXT_PUBLIC_GLASSBOX_W4` stay default OFF. No grade moves live.
- **181N note BANKED** — drafted + owner-approved-in-principle; parked for the batched launch (wording to be reconciled with the full GlassBox consumer story before it ships; Product positioning co-sign deferred to ship-time).
- **Go-live trigger moves to the batched GlassBox consumer launch** (after the remaining consumer-facing GlassBox work, e.g. W5). At that point: flip W4 + ship all notes + any later-wave grade moves together, as one owner-gated release (still tripwire #1).
F1 (materiality inert offline) + the HP×WFI watch-item remain logged for that future work.
