...
- **You are ALREADY running inside an isolated worktree at `C:\bari_wt_t449`** (branch `fix/task449-brined-inversion`, cut from origin/master `48811ebb`). Your repo root IS this worktree; engine at `03_operations\bsip2\proto_v0\src\`, website data at `bari-web\src\data\comparisons\`. Do NOT create another worktree. Do NOT touch `C:\Bari` (the shared main tree) — no `git stash`, no `git checkout` there. All edits, runs, and commits happen HERE.
- Read FIRST (read-only, from the main tree's absolute paths): `C:\Bari\tasks\TASK-449.md` (full D6/D7 verified diagnosis) and `C:\Bari\tasks\reports\TASK-449_brined_inversion_diagnosis_2026-07-02.md`.
...
- Commit locally on THIS worktree branch; **do NOT push, do NOT open a PR, do NOT deploy** — Content + Adversarial QA gates and the owner sit between you and the site.
...
```
Rules 5–8 of `C:\Bari\01_framework\operations\return_contract_v1.md` apply: full distributions with stdev, trace-derived counts with the deriving command, the Rule-7 flat table, and the full cross-corpus baseline diff.
