...
| Field | Value |
|---|---|
| Prompt file | `tasks\prompts\P46_lineage_viewer.md` |
| Model | `cursor/agent-cli` |
| Started | 2026-06-13T05:00:57.543815+00:00 |
| Finished | 2026-06-13T05:02:46.279972+00:00 |
| Exit code | 0 |
...
```
## P46 — spine.db lineage viewer

Created `03_operations/spine/show_lineage.py`, a read-only utility that prints the most recent e2e pipeline run from `spine.db`.

### Deliverable

**File:** `03_operations/spine/show_lineage.py`

- Opens `spine.db` read-only via `sqlite3.connect("file:...?mode=ro", uri=True)` (writes rejected)
- Resolves DB path relative to script location (works from any cwd)
- Stdlib only: `sqlite3`, `pathlib`, `json`, `sys`, `collections`
- Anchors on latest `merge_copy_and_gate` ok run; selects per-stage rows for that batch
- Prints 8 stages chronologically + grouped lineage chain (bsip0 → bsip1 → bsip2_trace → page → gate → copy)

### Command run

```bash
python 03_operations/spine/show_lineage.py
```
...
```
=== Stage runs (most recent e2e pipeline) ===
  extract_bsip0              status=ok       finished_at=2026-06-12T17:32:03+00:00
  build_bsip1                status=ok       finished_at=2026-06-12T17:32:03+00:00
  score_products             status=ok       finished_at=2026-06-12T17:32:03+00:00
  generate_page              status=ok       finished_at=2026-06-12T17:32:03+00:00
  gate_page                  status=ok       finished_at=2026-06-12T17:32:03+00:00
  build_copy_inputs          status=ok       finished_at=2026-06-12T17:32:04+00:00
  author_copy                status=ok       finished_at=2026-06-12T17:32:04+00:00
  merge_copy_and_gate        status=ok       finished_at=2026-06-12T17:32:04+00:00
...
=== Artifact lineage (child <- parent) ===
  ...
  [bsip0]
    03_operations\spine\_e2e_out\bsip0\bsip0_9990000000001.json
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_001.html
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_002.html
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_003.html
    03_operations\spine\_e2e_out\bsip0\bsip0_9990000000002.json
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_001.html
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_002.html
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_003.html
    03_operations\spine\_e2e_out\bsip0\bsip0_9990000000003.json
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_001.html
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_002.html
      <- 03_operations\spine\_fixtures_e2e\raw\raw_e2e_003.html

  [bsip1]
    03_operations\spine\_e2e_out\bsip1\bsip1_9990000000001.json
      <- 03_operations\spine\_e2e_out\bsip0\bsip0_9990000000001.json
      <- 03_operations\spine\_e2e_out\bsip0\bsip0_9990000000002.json
      <- 03_operations\spine\_e2e_out\bsip0\bsip0_9990000000003.json
```
...
| Check | Result |
|---|---|
| Read-only | `file:...?mode=ro` URI mode; `DELETE` raises `attempt to write a readonly database` |
| Stdlib only | No third-party imports |
| No DB writes | Script performs SELECT only |
| Cwd-independent | Verified from `/tmp` — same 8 stages printed |
...
```json
{
  ...
  "artifacts": [
    {
      "path": "03_operations/spine/show_lineage.py",
      "action": "created",
      "sha256": "531ba33f0f7db25758ea4f6443333e5244859997f3c02143c039b16a36c989e3"
    }
  ],
  "counts": {
    "file_created": "1/1",
    "writes_to_db": "0/0 (read-only SELECT only)",
    "new_deps": "0/0 (stdlib only)",
    "exit_code": "0/0 (acceptance command)",
    "e2e_stages_printed": "8/8 (latest merge_copy_and_gate batch)",
    "lineage_kind_sections": "6/6 (bsip0, bsip1, bsip2_trace, page, gate, copy)"
  },
  "commands_run": [
    {"cmd": "python 03_operations/spine/show_lineage.py", "exit_code": 0}
  ],
  "not_done": [],
  "self_check": "`python 03_operations/spine/show_lineage.py` exits 0 and prints 8 chronological stage_runs plus grouped lineage chain for the latest e2e run — observed exit 0, 8 stages, 6 lineage kind sections."
}
```
```
...
M .claude/agents/cc-agent.md
 ...
 M 05_command_center/command_center.json
 M 05_command_center/command_center_archive.json
 M 05_command_center/command_center_live.json
 M CLAUDE.md
...
?? .claude/commands/cc-agent.md
?? .claude/commands/orchestrate.md
?? .claude/hooks/inject-dispatch-signals.ps1
?? .claude/worktrees/
?? .github/
