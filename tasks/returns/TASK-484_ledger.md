...
| # | Field | Before | After | Decision |
|---|---|---|---|---|
| 1 | hero.title | "שוקולד מריר לא הופך לבריא בגלל השם — הציון מפריד בין..." | "בריאות השוקולד המריר נקבעת ביחס שבין קקאו לסוכר; הציון מפריד בין הקקאו האמיתי לבין הסוכר שלובש את השם" | REWORD — dropped "doesn't become healthy because of the name" negation; states positively what determines healthiness; em-dash dropped |
| 2 | prologueSentences[0] | "שוקולד מריר לא הופך לבריא בגלל השם: גם טבלת 70%..." | "בריאות השוקולד המריר נקבעת ביחס שבין קקאו לסוכר: גם טבלת 70%..." | REWORD — same fix applied consistently to the repeated phrase |
| 3 | categoryNote[0] | "...וגם שתי הטבלאות שמובילות את המדף לא מתחמקות מזה. ה-B הוא הצד הנכון של מדף הממתקים; לא מוצר בריאות." | "...וגם שתי הטבלאות שמובילות את המדף נושאות את זה במלואו. ה-B הוא הצד הטוב של מדף הממתקים, עדיין בתוך משפחת הממתקים." | REWORD — dropped "the right side of the candy shelf; not a health product" antithesis; restated positively |
...
| 4 | caveat.notes[1].body | "משקה שקדים, סויה או שיבולת שועל אינו מקבל ציון נמוך רק משום שאינו חלב. כל מוצר נמדד מול דומיו לפי ההרכב שעל האריזה — אך שימו לב..." | "משקה שקדים, סויה או שיבולת שועל נמדד מול דומיו לפי ההרכב שעל האריזה, בדיוק כמו חלב פרה. שימו לב..." | REWORD — dropped the double-negative "doesn't get a low score just because it isn't milk"; states positively that it's measured the same way as cow's milk; em-dash dropped |
