...
| Field | Value |
|---|---|
| Prompt file | `tasks\prompts\P261_labels_rank.md` |
| Model | `cursor/agent-cli` |
| Started | 2026-07-01T09:11:21.230568+00:00 |
| Finished | 2026-07-01T09:15:05.399516+00:00 |
| Exit code | 0 |
...
```
## P261 — Label remediation + rank reindex

Staging-only changes applied to the five comparison JSON files under `bari-web/src/data/comparisons/`. No scores were altered; no commit/push.

### 1. `cookies_coffee_frontend_v2.json`
| Metric | Before → After |
|--------|----------------|
| Products | 119 → **117** |
| Sort | Re-sorted by `score` desc; `rank` reindexed 1..117 |
| Discarded | **2** (material data missing) |
| Relabeled | **32** stale partial → full confidence |
| Kept partial | **31** (5 sugar-null + 26 fiber-only-null) |

**Discarded (evidence):**
- `7290017724171` — `sodium` null
- `7296073659969` — `protein`, `sodium` null

**Relabel target:** `confidence`=`verified`, `confidence_label_he`=`מבוסס על נתונים מלאים` (matching complete products in-file).

### 2. `cakes_hard_cookies_frontend_v1.json`
| Tier | Count | Action |
|------|-------|--------|
| A — missing material field | **1** | Discarded |
| B — complete data, stale partial label | **11** | Relabeled to `נתונים מלאים` / `verified` |
| C — fiber-only-null | **51** | Kept as partial |

**Discarded:** `5431920` — `sugar` null  
**Products:** 63 → **62**; ranks reindexed; `_meta` + hero counts updated.

### 3. Rank reindex only
| File | N | Rank mismatches fixed |
|------|---|----------------------|
| `cheese_frontend_v4.json` | 48 | 48 |
| `chocolate_bars_frontend_v1.json` | 23 | 2 |
| `milk_frontend_v1.json` | 18 | 8 |

### Verification
- All 5 files: monotonic-by-score ✓, `rank`=1..N ✓
- Score changes vs git HEAD on surviving barcodes: **0/117 cookies, 0/62 cakes, 0/48 cheese, 0/23 chocolate, 0/18 milk**
- Stale partial labels on complete material data: **0 remaining**

**Verification command:**
```bash
python C:/Bari/.tmp_p261_apply.py
```
(Report written to `C:/Bari/.tmp_p261_report.json`)
...
```json
{
  ...
  "artifacts": [
    {"path": "bari-web/src/data/comparisons/cookies_coffee_frontend_v2.json", "action": "modified", "sha256": "b2dee9792be284187a9ee8052d8eb333a2397c057d6d79d33dd33fa0336d66c6"},
    {"path": "bari-web/src/data/comparisons/cakes_hard_cookies_frontend_v1.json", "action": "modified", "sha256": "4c34716338a906711198ca072e9c0729e03aa8226d8ff9a4173246091e6219f7"},
    {"path": "bari-web/src/data/comparisons/cheese_frontend_v4.json", "action": "modified", "sha256": "a680fdaab47964378b02d54714455788aaaceaff1d424329b8d1c15b8aad7ef1"},
    {"path": "bari-web/src/data/comparisons/chocolate_bars_frontend_v1.json", "action": "modified", "sha256": "4e1ac56104a8a6ff1727fcd36df08b6c9b404d57be8aa8387cae893f3bb21a7b"},
    {"path": "bari-web/src/data/comparisons/milk_frontend_v1.json", "action": "modified", "sha256": "72e347de58713afb22f0049a51ffbd2f8137d359f591e26b5a78ec5a7730d6a8"}
  ],
  "counts": {
    "cookies_products: 117/117 (frontend JSON)": "117",
    "cookies_discarded: 2/119 (frontend JSON)": "2",
    "cookies_relabeled: 32/65 partial-label (frontend JSON)": "32",
    "cookies_kept_partial: 31/65 partial-label (frontend JSON)": "31",
    "cakes_products: 62/62 (frontend JSON)": "62",
    "cakes_tier_a_discard: 1/63 (frontend JSON)": "1",
    "cakes_tier_b_relabel: 11/63 (frontend JSON)": "11",
    "cakes_tier_c_keep: 51/63 (frontend JSON)": "51",
    "score_unchanged_common_barcodes: 268/268 (git HEAD diff)": "268",
    "rank_mismatches_fixed_cheese: 48/48 (frontend JSON)": "48",
    "rank_mismatches_fixed_chocolate: 2/23 (frontend JSON)": "2",
    "rank_mismatches_fixed_milk: 8/18 (frontend JSON)": "8"
  },
  "commands_run": [
    {"cmd": "python C:/Bari/.tmp_p261_apply.py", "exit_code": 0},
    {"cmd": "python (git HEAD score diff on 5 comparison JSONs)", "exit_code": 0}
  ],
  "not_done": [],
  "self_check": "All 5 files: rank=1..N, monotonic-by-score, 0 score changes on surviving barcodes; cookies 2 discarded + 32 relabeled; cakes 1 discarded + 11 relabeled + 51 fiber-only kept — observed PASS"
}
```
```
