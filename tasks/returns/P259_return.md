...
| Field | Value |
|---|---|
| Prompt file | `tasks\prompts\P259_repro_repair_granola_hc.md` |
| Model | `cursor/agent-cli` |
| Started | 2026-07-01T08:52:55.456086+00:00 |
| Finished | 2026-07-01T08:58:49.271077+00:00 |
| Exit code | 0 |
...
```
P259 / TASK-418 is complete — score-neutral repro repair for granola and hard_cheeses.

## Granola

**Root cause:** `granola.json` pointed `corpus_dirs` at `run_cereals_005`, where the two flagged products have `sugars_g: null`. The engine inferred high scores (+16/+20 vs committed). The enriched BSIP1 in `run_cereals_008` (sugars populated) reproduces committed scores with existing flags — no surgical score patch needed.

**Fixes:**
- `corpus_dirs`: `run_cereals_005` → `run_cereals_008`
- `live_manifest.json`: granola v1 → v2 (route already served v2)

**Target products (published unchanged):**

| Barcode | Before (engine) | After (engine) | Committed |
|---------|-----------------|----------------|-----------|
| 7290011668587 | 57.6 / C | **38.0 / D** | 38.0 / D |
| 7290014471443 | 51.6 / C | **35.5 / D** | 35.5 / D |

## Hard cheeses

**Root cause:** `baseline_json` had a JSON `\b` escape (`C:\Bari\bari-web` → backspace path). The v4 baseline file could not load; eval showed `matched=0`.

**Fixes:**
- Corrected path to `C:\Bari\bari-web\...`
- Manifest v2 → v4
- `run_products_dir` → `run_hc_task412_rt4_fix`

**After path fix:** 31/31 traces match v4 baseline — the eval `matched=0` was purely the path bug, not real score drift.

## Verification

- `conformance.py --slug granola` → **CONFORMS**
- `conformance.py --slug hard-cheeses` → **CONFORMS**
- OFF markers: **0**
- Staging only — no commit/push; no `bari-web` JSON touched

**Files modified:** `configs/granola.json`, `configs/hard_cheeses.json`, `conform_baseline.py`, `spine/live_manifest.json`

Return contract: `tasks/returns/P259_return.md`
```
