...
| Field | Value |
|---|---|
| Prompt file | `tasks\prompts\P55_data_missing_values_probe.md` |
| Model | `gemini/cli` |
| Started | 2026-06-13T12:13:00.099946+00:00 |
| Finished | 2026-06-13T12:16:25.559663+00:00 |
| Exit code | 0 |
...
```
Investigated 15 target products from `brined_cheeses_frontend_v2.json` (12 with `missing_ingredients`, 3 with `missing_nutrition`) to determine if data gaps were genuine or parser-induced.

### Investigation Results

| Barcode | Name | Missing Field | Raw Evidence (BSIP0/HTML) | Verdict |
| :--- | :--- | :--- | :--- | :--- |
| 2511229 | בולגרית מעודנת 16% | Ingredients | `ingredients_raw`: "" / No "רכיבים" in HTML | **HONEST-NULL** |
| 2511236 | בולגרית מעודנת 5% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 2511243 | בולגרית מסורתית 16% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 4861056 | גבינה צפתית 5% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 5992872 | גבינה צפתית במים 5% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 7290114310550 | פטה עיזים מעודנת 5% | Ingredients | Marketing copy instead of ingredients | **HONEST-NULL** (Cleansed) |
| 4861070 | גבינה צפתית קשה 24% | Ingredients | `ingredients_raw`: "" / No "רכיבים" in HTML | **HONEST-NULL** |
| 7296073644996 | גבינה צפתית קשה 24% מגורד | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 2385455 | בולגרית מעודנת 24% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 2107071 | פטה עיזים מעודנת 16% | Ingredients | Marketing copy instead of ingredients | **HONEST-NULL** (Cleansed) |
| 7296073730330 | פטינה בסגנון פטה 22% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 8606370 | פטינה גבינה סגנון פטה 22% | Ingredients | `ingredients_raw`: "" in BSIP0 | **HONEST-NULL** |
| 4861360 | גבינה צפתית בטעמים | Sugar/Fiber | `sugars_g`: null in BSIP1 | **HONEST-NULL** |
| 48413 | גבינה מלוחה חמד 16% | Sugar/Fiber | `sugar_raw`: "" in BSIP0 | **HONEST-NULL** |
| 369617 | כדורי פטה בשמן מתובל | Sugar/Fiber | `sugars_g`: null in BSIP1 | **HONEST-NULL** |

### Summary
- **Genuine Scrape Gaps (HONEST-NULL):** 15/15
- **Parser Misses (PARSER-MISS):** 0/15
- **Specifically Checked 4861070:** Confirmed that neither the BSIP0 raw JSON nor the source HTML contains an ingredient list. The parser is behaving correctly given the source data.
- **Marketing Bleed Fix:** Confirmed that barcodes `7290114310550` and `2107071` are nullified by `03_operations\bsip1\run_brined_cheeses_002\build_bsip1_002.py` due to non-nutritional marketing text in the source.
- **External Sources (OFF):** Confirmed `off_source_used: false` for all investigated products. **OFF=0**.

### Parser Identification
- **BSIP0 Parser:** Shufersal `html_parse` (logic seen in `_probe_shufersal_nutr.py`).
- **Build Hygiene:** `03_operations\bsip1\run_brined_cheeses_002\build_bsip1_002.py` (responsible for intentional nullification of marketing text).
...
```json
{
  ...
  "artifacts": [],
  "counts": {
    "honest_null_classification": "15/15 (brined_cheeses_frontend_v2.json targets)",
    "parser_miss_classification": "0/15 (brined_cheeses_frontend_v2.json targets)",
    "off_source_consulted": "0/15 (brined_cheese_bsip0_raw_20260613T065721.json)"
  },
  "commands_run": [
    {"cmd": "grep \"4861070\" 02_products\\brined_cheeses\\bsip0_outputs\\brined_cheese_bsip0_raw_20260613T065721.json", "exit_code": 0},
    {"cmd": "grep \"רכיבים\" 03_operations\\bsip0\\raw_store\\shufersal\\brined_cheeses\\P_4861070\\20260613T070140289825.html", "exit_code": 1}
  ],
  "not_done": [],
  "self_check": "Investigation of barcode 4861070 in raw HTML showed no 'רכיבים' tag, confirming HONEST-NULL status observed in frontend."
}
```
...
(node:16632) [DEP0190] DeprecationWarning: Passing args to a child process with shell option true can lead to security vulnerabilities, as the arguments are not escaped, only concatenated.
(Use `node --trace-deprecation ...` to show where the warning was created)
Attempt 1 failed: You have exhausted your capacity on this model. Your quota will reset after 0s.. Retrying after 5937ms...
Attempt 2 failed: You have exhausted your capacity on this model. Your quota will reset after 0s.. Retrying after 10145ms...
```
