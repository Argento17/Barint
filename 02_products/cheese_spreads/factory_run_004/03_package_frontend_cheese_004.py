"""
TASK-169B — Frontend packaging for cottage / white cheese (run_cheese_004,
BARI_RECAL_P0=ON recalibration).

Builds frontend_package.json from the run_cheese_004 BSIP2 traces + the SAME
BSIP1 cheese governance constructs as run_cheese_003 (same corpus). Identical
curation logic to 03_package_frontend_cheese_003.py — only the trace source dir,
output path, run_id, engine string and provenance change. Scores/grades come
straight from the recalibrated traces; no manual edits.

Output: 02_products/cheese_spreads/factory_run_004/frontend_package.json
"""
...
OUT = pathlib.Path(r"C:\Bari\02_products\cheese_spreads\factory_run_004\frontend_package.json")
...
for tf in TRACES:
    ...
    # NOTE: under RECAL_P0 the A-ceiling withhold rule still applies as designed —
    # a grade-A (or S) product that is NOT A-eligible (C1-C6) is withheld. The recal
    # raises legitimately-eligible products INTO A, so fewer A's are withheld than 003.
    a_ceiling_withhold = (grade in ("A", "S") and ac.get("a_eligible_pre_routing") is False)
...
package = {
    ...
    "non_authoritative_reason": "run_cheese_004 = run_cheese_003 corpus re-scored with BARI_RECAL_P0=ON (TASK-169B P1 promotion of the owner-approved P0 recalibration R1-R7). NOT yet promoted to live: staged for final owner look. Flag-OFF rollback is byte-identical (golden+router regressions green; OFF==model OFF baseline). EV-030/031/032/033 + EV-027/EV-024-026 extensions authored in the evidence registry.",
    "rtl": True,
    "language": "he",
    "engine": "proto_v0 / 0.4.1 + BARI_RECAL_P0=on",
    "recal_p0": "on",
    "governance_ref": "TASK-169 recalibration (R1-R7 v1.1); cheese_spreads_stress_test_001 (TASK-141, verdict B)",
    "counts": {
        "total": len(products),
