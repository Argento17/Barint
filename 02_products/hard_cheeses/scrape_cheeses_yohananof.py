"""
Scrape approved hard cheese candidates from yochananof.co.il — TASK-215 nutrition source fix.

Adapts 03_operations/bsip0/scrape/yohananof/03_scrape_yohananof.py for the
hard_cheeses category. Reads approved_candidates.json from yohananof_scrape/,
opens product modals, captures nutrition / ingredients / allergens HTML per barcode.

Run after discover_cheeses_yohananof.py + auto_approve_cheeses.py:
    python scrape_cheeses_yohananof.py
"""
...
def load_approved():
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run discover_cheeses_yohananof.py first, then run the auto-approve step."
        )
...
def main():
    ...
    print("\n==============================")
    print(f"Approved hard cheese products to scrape: {len(approved)}")
    print("==============================")
