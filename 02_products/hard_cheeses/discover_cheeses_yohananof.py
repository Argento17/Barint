"""
Discover hard cheese candidates on yochananof.co.il — TASK-215 nutrition source fix.

Adapts 03_operations/bsip0/scrape/yohananof/01_discover_yohananof.py for the
hard_cheeses category. Outputs candidate list to yohananof_scrape/; auto-approve step
is in a separate helper (run after this).

Run:
    python discover_cheeses_yohananof.py
"""
...
HARD_EXCLUDE = [
    "גבינה לבנה", "קוטג'", "לבנה", "שמנת",
    "מסקרפונה", "ריקוטה", "גבינת שמנת",
    "גבינה מעובדת פרוסות", "ממרח", "גבינה חריפה",
]

POSITIVE_TYPES = [
    "גבינה צהובה", "גבינה קשה", "גאודה", "קשקבל",
    "אמנטל", "גבינה בולגרית", "גבינה צפתית", "פרמזן",
]
...
def candidate_decision(product):
    ...
    # Accept anything with "גבינה" that wasn't hard-excluded — REVIEW for human/auto triage
    has_cheese = "גבינה" in combined or "עמק" in combined

    if positive_type:
        return "YES", "positive_cheese_type"

    if has_cheese:
        return "REVIEW", "cheese_needs_review"

    return "REJECT", "not_hard_cheese_type"
...
def mark_product_cards(page):
    ...
    () => {
      ...
      function score(el) {
        ...
        let s = 0;
        if (txt.includes('₪')) s += 3;
        if (txt.includes('גרם') || txt.includes("גר'") || txt.includes('ק"ג') || txt.includes('קג')) s += 2;
        if (txt.includes('גבינה') || txt.includes('גאודה') || txt.includes('עמק') || txt.includes('קשקבל')) s += 3;
...
def choose_best_name(card_text, image_alt):
    ...
    for line in candidates:
        if any(t in line for t in ["גבינה", "גאודה", "עמק", "קשקבל", "אמנטל", "פרמזן", "בולגרית", "צפתית"]):
            return line
